...
This example trains a tiny contextual token model from 1,666 annotated examples. It recognizes amounts, currencies, and modifiers, then applies an application-owned deterministic decoder.
...
The supported domain is English text, nonnegative money, English decimal notation, number words below 100, thousand/million modifiers, and explicit EUR/USD/GBP/SEK. The demo interprets a bare `$` as USD. Unknown words, unsupported formats, and ambiguous multiple amounts abstain. The learned model distinguishes monetary numbers from nearby invoice IDs and years in the controlled examples.
