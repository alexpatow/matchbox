...
const decode: SequenceDecoder = (tokens) => {
  ...
  if (tokens.some((t) => t.label === "TODAY" || t.label === "TOMORROW")) {
    return decodeClock(tokens);
  }
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (["O", "DURATION", "RELATIVE"].includes(token.label)) {
      continue;
    }
  ...
  if (!pairs || !Number.isFinite(seconds) || seconds > 604800) {
    return null;
  }
