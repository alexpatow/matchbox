...
export function decodeClock(tokens: readonly TaggedToken[]) {
  ...
  if (days.length !== 1 || !clock.length || clock.length > 2 || periods.length > 1) return null;
  if (
  ...
  if (
    ...
    return null;
  if (tokens.filter((t) => t.label === "COLON").length !== clock.length - 1) return null;
  if (periods.length ? hour < 1 || hour > 12 : hour > 23) return null;
  return {
    kind: "datetime",
