...
export async function packageModel(
  output: string,
  model: SequenceArtifact | RecordArtifact | RecurrentArtifact | FeatureArtifact,
  report: unknown,
) {
  ...
  const encoderImport =
    model.kind === "feature-parser"
      ? `import encode from ${JSON.stringify(model.encoderModule.replace(/\.ts$/, ""))};\n`
      : "";
  let adapter = model.decoderModule === null ? "" : ", decode";
  if (model.kind === "feature-parser") {
    adapter = ", encode";
  }
  const inputType = model.kind === "recurrent-parser" ? "" : ", InferInput<typeof task>";
  ...
  await writeFile(
    declaration,
    `import task from ${JSON.stringify(taskImport)};\nimport type { InferOutput, InferInput } from "@matchbox-ai/core";\nimport type { ${parserType} } from "@matchbox-ai/core/runtime";\ndeclare const parser: ${parserType}<InferOutput<typeof task>${inputType}> & { load(): Promise<void>; dispose(): void };\nexport default parser;\n`,
  );
