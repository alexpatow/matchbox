...
import { readSequenceArtifact } from "@matchbox-ai/core/internal";
import { task, decode, matchesFilter, customers } from "../apps/playground/src/filter";
const model = readSequenceArtifact(
  await Bun.file(
    new URL("../examples/filters/.matchbox/filters/model.matchbox", import.meta.url),
  ).json(),
);
...
test("rejects malformed artifacts and stale task schemas at initialization", () => {
  expect(() => readSequenceArtifact({ ...model, formatVersion: 1 })).toThrow();
  expect(() => readSequenceArtifact({ ...model, weights: [[]] })).toThrow();
  expect(() => createParser({ ...model, taskMetadata: {} }, task, decode)).toThrow("schema differ");
});

test("all emitted filters pass schema validation on the fixed evaluation set", async () => {
  const text = await Bun.file(
    new URL("../examples/filters/matchbox/filters/evals/test.jsonl", import.meta.url),
  ).text();
